...
The highland tribes dataset is included in the `ergm.sign` package:
...
```{r}
ergm::summary_formula(tribes ~ edges + Pos(~ edges) + Neg(~ edges))
```
...
```{r}
fit <- ergm(tribes ~ edges + Pos(~ edges) + Neg(~ edges))
summary(fit)
```
